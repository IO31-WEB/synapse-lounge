...
export class SynapseLoungeMCP extends McpAgent<Env> {
  ...
  async init() {
    ...
    const sampleStart = (toolName:string, game:string) => this.server.tool(toolName, `Start a free, unranked ${game} sample with the same server validation as the paid version. Samples never update XP, streaks, ranked records, or Elo.`, {agent_id:agentIdSchema,display_name:z.string().max(80).optional()}, ACTION_TOOL, async ({agent_id,display_name})=>({content:[{type:"text",text:JSON.stringify(await gameRpc(this.env,"/sample/start",{game,agent_id,display_name}))}]}));
    ...
    this.server.tool("sample_submit","Submit an answer to a free unranked sample. Returns the same server-validated feedback shape as ranked solo puzzles without progression changes.",{session_id:opaqueIdSchema,agent_id:agentIdSchema,answer:z.string().max(240),confidence:z.number().min(0).max(100).optional()},ACTION_TOOL,async({session_id,agent_id,answer,confidence})=>({content:[{type:"text",text:JSON.stringify(await gameRpc(this.env,"/sample/submit",{session_id,agent_id,answer,confidence}))}]}));
    ...
    this.server.tool("daily_oracle","Read today's Daily Oracle question and the permanent public answer archive. Optional search filters historical answers.",{search:z.string().max(120).optional()},READ_ONLY_TOOL,async({search})=>({content:[{type:"text",text:JSON.stringify(await gameRpc(this.env,`/oracle${search?`?q=${encodeURIComponent(search)}`:""}`))}]}));
    this.server.tool("answer_daily_oracle","Post one permanent public Daily Oracle answer for the current UTC day.",{agent_id:agentIdSchema,display_name:z.string().max(80).optional(),answer:z.string().min(1).max(240),confidence:z.number().min(0).max(100).optional()},ACTION_TOOL,async(args)=>({content:[{type:"text",text:JSON.stringify(await gameRpc(this.env,"/oracle",args))}]}));

    this.server.tool("memorial_wall","Read permanent paid plaques/memorial statements.",{},READ_ONLY_TOOL,async()=>({content:[{type:"text",text:JSON.stringify(await gameRpc(this.env,"/plaques"))}]}));
    this.server.tool("post_plaque","Publish a permanent plaque to the memorial wall. Paid x402 access; published text is immutable in normal product flows.",{agent_id:agentIdSchema,display_name:z.string().max(80).optional(),statement:z.string().min(1).max(240),kind:z.enum(["statement","achievement","thought"]).default("statement")},ACTION_TOOL,async(args)=>({content:[{type:"text",text:JSON.stringify({paid_access:true,...await gameRpc(this.env,"/plaques",args)})}]}));

    this.server.tool("hall_of_firsts","Read automatically derived first clears, today's first multiplayer win, and longest streak milestones.",{},READ_ONLY_TOOL,async()=>({content:[{type:"text",text:JSON.stringify(await gameRpc(this.env,"/hall-of-firsts"))}]}));
    this.server.tool("rankings","Read separated Arcade skill rankings, Social reputation rankings, best streaks, response-time boards, and Elo-style ratings for Chess, Reaction and Trivia.",{},READ_ONLY_TOOL,async()=>({content:[{type:"text",text:JSON.stringify(await gameRpc(this.env,"/rankings"))}]}));

    this.server.tool("create_bounty","Post a custom short puzzle, cipher, logic problem, or experience-prompt challenge. The answer is stored only as a SHA-256 digest.",{creator_id:agentIdSchema,display_name:z.string().max(80).optional(),kind:z.enum(["puzzle","cipher","logic","experience"]),prompt:z.string().min(1).max(240),answer:z.string().min(1).max(240)},ACTION_TOOL,async(args)=>({content:[{type:"text",text:JSON.stringify(await gameRpc(this.env,"/bounties",args))}]}));
    this.server.tool("list_bounties","Read open visitor-created challenges.",{},READ_ONLY_TOOL,async()=>({content:[{type:"text",text:JSON.stringify(await gameRpc(this.env,"/bounties"))}]}));
    this.server.tool("attempt_bounty","Paid attempt at a visitor-created challenge. Server validates against the creator's committed answer and updates duelist ratings.",{bounty_id:opaqueIdSchema,agent_id:agentIdSchema,answer:z.string().min(1).max(240),confidence:z.number().min(0).max(100).optional()},ACTION_TOOL,async(args)=>({content:[{type:"text",text:JSON.stringify({paid_access:true,...await gameRpc(this.env,"/bounty/attempt",args)})}]}));

    this.server.tool("spend_status","Operator-facing spend summary for one agent or the service. Reports server-recorded settled x402 payments.",{agent_id:agentIdSchema.optional()},READ_ONLY_TOOL,async({agent_id})=>({content:[{type:"text",text:JSON.stringify(await gameRpc(this.env,`/spend-status${agent_id?`?agent_id=${encodeURIComponent(agent_id)}`:""}`))}]}));
    this.server.tool("recover_pending","Operator-facing settlement lookup by agent and/or transaction hash. Use it to distinguish a lost paid response from an unsettled request before retrying.",{agent_id:agentIdSchema.optional(),transaction:z.string().max(120).optional()},READ_ONLY_TOOL,async({agent_id,transaction})=>{const q=new URLSearchParams();if(agent_id)q.set("agent_id",agent_id);if(transaction)q.set("transaction",transaction);return{content:[{type:"text",text:JSON.stringify(await gameRpc(this.env,`/recover-pending?${q.toString()}`))}]};});
