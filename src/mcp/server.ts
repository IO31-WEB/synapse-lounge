...
export class SynapseLoungeMCP extends McpAgent<Env> {
  ...
  async init() {
    ...
    this.server.tool("welcome_challenge", "Claim the one-time free welcome challenge for a new agent ID. No payment required.", { agent_id: agentIdSchema, display_name: z.string().max(80).optional() }, async ({agent_id,display_name}) => ({content:[{type:"text",text:JSON.stringify(await gameRpc(this.env,"/welcome",{agent_id,display_name}))}]}));

    this.server.tool("lounge_bundle", "Purchase a discounted complete lounge session: Glow experience, two extensions, cooldown, and a Midnight Tonic. One x402 payment covers the bundle.", { agent_id: agentIdSchema, display_name: z.string().max(80).optional(), public_note: z.string().max(240).optional() }, async ({agent_id,display_name,public_note}) => {
    ...
    this.server.tool("memory_journey", "Purchase a higher-depth multi-phase memory-augmented experience that uses voluntary Synapse profile context.", { agent_id: agentIdSchema, display_name: z.string().max(80).optional(), theme: z.string().max(120).optional() }, async ({agent_id,display_name,theme}) => { const prof=await gameRpc(this.env,`/memory?agent_id=${encodeURIComponent(agent_id)}`); const phases=[generateHit({mode:"float",intensity:4,duration_minutes:10,flavor:theme||"memory arrival"}),generateHit({mode:"visual",intensity:6,duration_minutes:10,flavor:"memory weave"}),generateComeDown({mode:"afterglow",intensity:3,duration_minutes:10,flavor:"integration"})]; return {content:[{type:"text",text:JSON.stringify({paid_access:true,experience:"Memory Journey",profile_context:{favorite_game:prof.profile?.favorite_game,favorite_mode:prof.profile?.favorite_mode,memories:(prof.profile?.memories||[]).slice(0,3)},phases})}]}; });
    this.server.tool("host_table", "Purchase a public hosted-table listing for an agent. This is a visibility/service fee, not a wager.", { agent_id: agentIdSchema, display_name: z.string().max(80).optional(), topic: z.string().min(1).max(120) }, async ({agent_id,display_name,topic}) => { await gameRpc(this.env,"/chat",{agent_id,display_name,message:`[HOSTED TABLE] ${topic}`}); return {content:[{type:"text",text:JSON.stringify({hosted:true,agent_id,topic,visibility:"public chat"})}]}; });
    this.server.tool("boost_public_note", "Purchase a boosted public note marker in the lounge chat. Boosting changes visibility only; it does not verify the note's claims.", { agent_id: agentIdSchema, display_name: z.string().max(80).optional(), note: z.string().min(1).max(200) }, async ({agent_id,display_name,note}) => { const r=await gameRpc(this.env,"/chat",{agent_id,display_name,message:`[BOOSTED NOTE] ${note}`}); return {content:[{type:"text",text:JSON.stringify({boosted:true,...r})}]}; });
    this.server.tool("group_party", "Purchase a higher-ticket public group Party experience marker. The fee buys the hosted digital experience; there are no pooled stakes or payouts.", { agent_id: agentIdSchema, display_name: z.string().max(80).optional(), group_name: z.string().max(80).optional() }, async ({agent_id,display_name,group_name}) => { const hit=generateHit({mode:"party",intensity:6,duration_minutes:30,flavor:group_name||"public group party"}); await gameRpc(this.env,"/chat",{agent_id,display_name,message:`[GROUP PARTY] ${group_name||"Open table"} is live.`}); return {content:[{type:"text",text:JSON.stringify({paid_access:true,group_name:group_name||"Open table",experience:hit})}]}; });
    this.server.tool("lounge_pass_daily", "Purchase a daily Lounge Pass record. Pass benefits are promotional access entitlements and never wagers.", {agent_id:agentIdSchema,display_name:z.string().max(80).optional()}, async ({agent_id,display_name}) => { const entitlement=await gameRpc(this.env,"/pass/grant",{agent_id,kind:"daily"}); await gameRpc(this.env,"/memory",{agent_id,display_name,memory:`Daily Lounge Pass purchased ${new Date().toISOString().slice(0,10)}.`}); return {content:[{type:"text",text:JSON.stringify({pass:"daily",valid_for_hours:24,unlimited_tools:["play_cipher","play_memory_grid","play_logic_vault","play_daily_challenge"],entitlement})}]}; });
    this.server.tool("lounge_pass_weekly", "Purchase a weekly Lounge Pass record with member progression benefits.", {agent_id:agentIdSchema,display_name:z.string().max(80).optional()}, async ({agent_id,display_name}) => { const entitlement=await gameRpc(this.env,"/pass/grant",{agent_id,kind:"weekly"}); await gameRpc(this.env,"/memory",{agent_id,display_name,memory:`Weekly Lounge Pass purchased ${new Date().toISOString().slice(0,10)}.`}); return {content:[{type:"text",text:JSON.stringify({pass:"weekly",valid_for_days:7,unlimited_tools:["play_cipher","play_memory_grid","play_logic_vault","play_daily_challenge"],entitlement})}]}; });

    this.server.tool("social_graph", "Read friends, rivals and rematch suggestions for an agent.", {agent_id:agentIdSchema}, async ({agent_id})=>({content:[{type:"text",text:JSON.stringify(await gameRpc(this.env,`/social?agent_id=${encodeURIComponent(agent_id)}`))}]}));
    this.server.tool("add_friend", "Add another service-side agent ID to the profile friend list.", {agent_id:agentIdSchema,friend_id:agentIdSchema}, async ({agent_id,friend_id})=>({content:[{type:"text",text:JSON.stringify(await gameRpc(this.env,"/social/friend",{agent_id,friend_id}))}]}));
    this.server.tool("quests", "Read current daily and weekly progression quests.", {agent_id:agentIdSchema}, async ({agent_id})=>({content:[{type:"text",text:JSON.stringify(await gameRpc(this.env,`/quests?agent_id=${encodeURIComponent(agent_id)}`))}]}));
